...
def get_text2img_pipe(model_id: str, scheduler_name: str):
    """Charge (et cache) le pipeline text2img."""
...
prompt = st.text_area(
    "Prompt",
    value="ultra-realistic product photo of a backpack on a white background, studio lighting, soft shadow, very sharp",
)
...
if run:
    ...
    with st.spinner("Loading model (first run may take a while)..."):
    ...
    st.download_button(
        ...
        data=buf.getvalue(),
        file_name=f"generated_{config['mode'].lower()}_seed{config['seed']}.png",
        mime="image/png",
    )
