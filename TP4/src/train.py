...
def main() -> None:
    ...
    # --- NeighborLoader for GraphSAGE (requires pyg-lib or torch-sparse) ---
    # On Mac ARM, these packages are unavailable, so we fall back to full-batch.
    use_sampling = False
    train_loader = None
    if args.model == "sage":
        try:
            ...
            # trigger one batch to check if sampling backend is available
            _ = next(iter(train_loader))
            use_sampling = True
            print("sampling: NeighborLoader (mini-batch)")
        except (ImportError, Exception) as e:
            print(f"sampling: NeighborLoader unavailable ({e}), falling back to full-batch")
            train_loader = None
            use_sampling = False
    ...
    for epoch in range(1, epochs + 1):
        ...
        if args.model in ["mlp", "gcn"] or (args.model == "sage" and not use_sampling):
            with Timer() as t:
                if args.model == "mlp":
                    logits = model(x)
                else:
