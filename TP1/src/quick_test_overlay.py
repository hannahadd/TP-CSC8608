...
def main():
    # --- Choix image ---
    imgs = (
        sorted(Path("TP1/data/images").glob("*.jpg"))
        + sorted(Path("TP1/data/images").glob("*.jpeg"))
        + sorted(Path("TP1/data/images").glob("*.png"))
    )
    if len(imgs) == 0:
        raise FileNotFoundError("Aucune image trouvée dans TP1/data/images (jpg/jpeg/png).")
    ...
    out_path = out_dir / f"overlay_{img_path.stem}_i{IMG_IDX}.png"
