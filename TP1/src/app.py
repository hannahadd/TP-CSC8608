...
with st.expander("Infos modèle", expanded=False):
    st.write({"model_type": MODEL_TYPE, "checkpoint": CKPT_PATH})
...
with col1:
    x1 = st.slider("x1", 0, W - 1, 0, key="x1")
with col2:
    y1 = st.slider("y1", 0, H - 1, 0, key="y1")
with col3:
    x2 = st.slider("x2", 0, W - 1, W - 1, key="x2")
with col4:
    y2 = st.slider("y2", 0, H - 1, H - 1, key="y2")
...
# Guidage points (Exo 7)
st.subheader("Guidage (optionnel) : points FG/BG")

c1, c2, c3 = st.columns(3)
with c1:
    px = st.slider("point x", 0, W - 1, int(W * 0.5), key="px")
with c2:
    py = st.slider("point y", 0, H - 1, int(H * 0.5), key="py")
with c3:
    ptype = st.selectbox("type", ["FG (objet)", "BG (fond)"], key="ptype")
...
# Preview bbox + points (avant segmentation)
...
# 3) Segmentation (stocker last_pred)
...
# 4) Sélection multimask + affichage résultat
...
if lp is not None and lp["img_name"] == img_name:
    ...
    idx = st.selectbox("index du masque", list(range(len(scores))), index=default_idx, key="mask_idx")
